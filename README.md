...
**Start here — you may not need to build a template at all.** This repo ships **ready-to-import, EJBCA-ready** templates (the full-DN Subject already applied) under [`Templates/EJBCA/`](./Templates/README.md). Import one directly and you can skip the preparation steps in this section entirely — nothing has to be installed or edited locally with `certutil`. For example, for domain controllers:
