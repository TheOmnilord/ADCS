...
- **Per-run timestamped log file** (`CertBatch_yyyyMMdd_HHmmss_<id>.log`, unique per run, written beside the tracking file)
...
- A timestamped log file (`CertBatch_yyyyMMdd_HHmmss_<id>.log`, the suffix random so runs that share a folder never collide) is created beside the tracking file for each run — not in the working directory, which is never validated. A missing `-OutputFolder` is created one component at a time with an atomic fail-on-existing create, so a junction planted in the missing part of the path after the folder check is refused rather than created through; every parent is judged *before* a folder is created in it — including what the new folder would inherit, because an inheritable grant on the parent becomes effective on the new folder the instant it exists — and a component ending in a space or a period is refused anywhere a folder is validated, because Windows resolves such a name differently as a leaf and as a parent. A folder that does not exist yet is refused — rather than created — when its parent would hand an untrusted principal rights on it; the `C:\` root does (an inheritable `CreateFiles` grant to Users), so pre-create a folder directly under `C:\` with a protected ACL, or pass `-AllowUnprotectedOutputFolder`. certreq's console output is captured in files beside the log, never under `%TEMP%` (which a SYSTEM or service run shares with every local user).
