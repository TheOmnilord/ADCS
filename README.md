...
| `-AllowLinkedIssuancePolicy` | switch | No | | Import/Sync: accept an issuance policy OID (`msPKI-Certificate-Policy` / `msPKI-RA-Policies`) that the *target* forest already links to a group via Authentication Mechanism Assurance. Refused by default, because certificates from the copy would grant that group's membership at logon to everyone the copy's enrollment ACL admits. |
