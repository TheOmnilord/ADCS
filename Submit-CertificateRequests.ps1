...
.NOTES
    ...
    - The script writes the tracking file after every request, not only at the end of the run. So a
      crash or an interruption in the middle of a batch keeps the RequestID of every request the CA
...
function Export-TrackingData {
    ...
    try {
        ...
        # File.Replace / File.Move rather than Move-Item -Force: a FOLDER occupying the tracking-file
        # name fails the write (Move-Item -Force would instead move the CSV INTO that folder). A LINK
        # at the tracking name is not judged here - Test-Path -PathType Leaf is true for a symbolic
        # link to a file - but Resolve-TrackingFilePath already refuses a hard-linked or symlinked
        # tracking file before the run starts, so one cannot reach this point.
        # [NullString]::Value, not $null: File.Replace's third argument (the backup file) is
        # optional, but PowerShell marshals a bare $null for a [string] parameter as an EMPTY
        # string, which File.Replace rejects ("The path is not of a legal form." on 5.1, "The path
        # is empty." on 7) - so every checkpoint after the first (the tracking file now exists, so
        # this branch runs) threw and aborted the run. [NullString]::Value passes a real .NET null.
        if (Test-Path -LiteralPath $Path -PathType Leaf) { [System.IO.File]::Replace($tempFile, $Path, [NullString]::Value) }
